...
export function missingFields(data: AIExtraction): string[] {
  ...
  if (!data.counterpartyName) fields.push(data.type === "purchase_order" ? "fornecedor" : "cliente");
  ...
  if (data.type === "purchase_order" && !data.deliveryAddress) fields.push("endereço de entrega");
  return fields;
}
