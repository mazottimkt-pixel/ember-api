...
export async function POST(request: Request) {
  ...
  if (!verifyMetaSignature(rawBody, request.headers.get("x-hub-signature-256"), process.env.META_APP_SECRET ?? "")) return Response.json({ error: "Assinatura inválida" }, { status: 401 });
  let payload: unknown; try { payload = JSON.parse(rawBody); } catch { return Response.json({ error: "JSON inválido" }, { status: 400 }); }
  ...
  return Response.json({ received: true, mode: process.env.WHATSAPP_ACCESS_TOKEN ? "configured" : "mock", payloadAccepted: Boolean(payload) });
}
